# 2025-05-23 16:46:33 by RouterOS 7.16.2
...
add allowed-address=10.99.99.2/32 interface=wg0 name=peer3 persistent-keepalive=\
    25s public-key="2K9DWoeJgP6LMmFZMedmaZagda8EnmxbR5kAc/F+0iE="
/ip address
...
add address=192.168.88.16 client-id=1:a8:a1:59:51:70:f3 mac-address=\
    A8:A1:59:51:70:F3 server=defconf
add address=192.168.88.32 client-id=1:60:45:cb:9b:d4:6 mac-address=\
    60:45:CB:9B:D4:06 server=defconf
...
add address=192.168.88.32 name=authentik.maxilabs.nl type=A
...
add action=drop chain=forward comment="defconf: drop invalid" connection-state=\
    invalid
add action=accept chain=forward comment="Allow HTTPS to 192.168.88.32" \
    dst-address=192.168.88.32 dst-port=443 in-interface-list=WAN protocol=tcp \
    src-address-list=https_whitelist
add action=drop chain=forward comment="Drop non-whitelisted HTTPS traffic" \
    dst-address=192.168.88.32 dst-port=443 in-interface-list=WAN protocol=tcp
add action=accept chain=forward comment="Allow Enshrouded UDP" dst-address=\
    192.168.88.32 dst-port=15637 in-interface-list=WAN protocol=udp
add action=accept chain=forward comment="Allow Enshrouded TCP" dst-address=\
    192.168.88.32 dst-port=15637 in-interface-list=WAN protocol=tcp
add action=drop chain=forward comment="defconf: drop all from WAN not DSTNATed" \
    connection-nat-state=!dstnat connection-state=new in-interface-list=WAN
add action=accept chain=input comment="Allow WireGuard handshake" dst-port=51830 \
    protocol=udp
add action=accept chain=forward comment="Allow VPN clients to communicate" \
    src-address=10.99.99.0/24
add action=accept chain=input comment="Allow VPN clients to reach router" \
    in-interface=wg0 src-address=10.99.99.0/24
...
add action=drop chain=input comment="defconf: drop all not coming from TRUSTED" \
    in-interface-list=!TRUSTED
/ip firewall mangle
add action=change-mss chain=forward comment="Clamp MSS over VPN" new-mss=1360 \
    out-interface=wg0 protocol=tcp tcp-flags=syn
add action=accept chain=forward comment="Allow clean TCP teardown" protocol=tcp \
    tcp-flags=fin,rst
/ip firewall nat
add action=masquerade chain=srcnat comment="defconf: masquerade" ipsec-policy=\
    out,none out-interface-list=WAN
add action=dst-nat chain=dstnat comment="Forward HTTPS to 192.168.88.32" \
    dst-port=443 in-interface-list=WAN protocol=tcp to-addresses=192.168.88.32
add action=dst-nat chain=dstnat comment="Enshrouded TCP" dst-port=15637 \
    in-interface-list=WAN protocol=tcp to-addresses=192.168.88.32
add action=dst-nat chain=dstnat comment="Enshrouded UDP" dst-port=15637 \
    in-interface-list=WAN protocol=udp to-addresses=192.168.88.32
add action=masquerade chain=srcnat comment=\
    "Allow VPN clients to access the internet" out-interface-list=WAN \
    src-address=10.99.99.0/24
add action=log chain=srcnat log-prefix="WG-NAT: " out-interface-list=WAN \
    src-address=10.99.99.0/24
...
add action=accept chain=forward comment=\
    "defconf: accept all that matches ipsec policy" ipsec-policy=in,ipsec
add action=drop chain=input comment="Drop all IPv6 not from TRUSTED" \
    in-interface-list=!TRUSTED
/system clock
